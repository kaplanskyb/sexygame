...
export default function TruthAndDareApp() {
  ...
  const startRound = async () => {
    ...
    if (!id) { alert('No challenges found. Try re-uploading CSV.'); return; }
  ...
  // --- UPLOAD CORREGIDO PARA GUARDAR SEXO ---
  const handleUploadCsv = async (e, collectionName) => {
    ...
    reader.onload = async (event) => {
      ...
      for (const line of lines) {
        ...
        if (collectionName === 'challenges' && cols.length >= 4) {
            // Normalizar type 'dare' -> 'D'
            let rawType = cols[1].trim().toLowerCase();
            let cleanType = rawType.includes('dare') ? 'D' : rawType.includes('truth') ? 'T' : rawType.toUpperCase();
            
            await addDoc(ref, {
                level: cols[0].trim(),
                type: cleanType,
                text: cols[2].trim(), // Pregunta
                sexo: cols[3].trim().toUpperCase(), // GUARDAMOS EL SEXO
                answered: false
            });
        }
        else if (collectionName === 'pairChallenges' && cols.length >= 4) {
            let rawType = cols[3].trim().toUpperCase();
            let cleanType = rawType === 'D' ? 'direct' : rawType === 'I' ? 'inverse' : 'direct';
            await addDoc(ref, {
                level: cols[0].trim(),
                male: cols[1].trim(),
                female: cols[2].trim(),
      ...
      setUploading(false); alert('Upload complete. Database updated.');
    };
  ...
  // --- VISUALIZACIÓN QUE RESPETA EL GÉNERO ---
  const getCardText = (c) => {
    if (!c) return 'Loading...'; 
    if (gameState?.mode === 'yn') {
    ...
    // Truth/Dare
    if (c.sexo && c.sexo !== 'B') {
        // Opcional: mostrar aviso si el género no coincide
        // const myGender = ...;
        // if (c.sexo === 'M' && myGender !== 'male') ...
        // Por ahora devolvemos el texto que es lo importante
        return c.text;
    }
